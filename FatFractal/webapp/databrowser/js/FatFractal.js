    ...
    this.getArrayFromUri = function(collectionUri, successCallback, errorCallback) {
        ...
        m_ajax({
            ...
            success: function(response) {
                ...
                var thingsDone = 0;
                var realRetVal = [];
                function incrThingsDoneAndCallSuccessCallbackIfAppropriate(n) {
                    n = n || 1;
                    for (var i = 0; i < n; i++) {
                        thingsDone++;
                        if (m_debug) console.log("thingsDone: " + thingsDone + "/" + thingsToDo);
                ...
                for (var i = 0; i < tmpRetVal.length; i++) {
                    if (!tmpRetVal[i].ffUrl) { // not an object under management by FatFractal - probably an extension result
                        ...
                        incrThingsDoneAndCallSuccessCallbackIfAppropriate(2);
                    }
                    else { // object under management by FatFractal
                        m_processResultObject(tmpRetVal[i], function() {
                            incrThingsDoneAndCallSuccessCallbackIfAppropriate();
                        });
            ...
            if (referringObject.ffRefs != null) {
                ...
                for (var refItemIx = 0; refItemIx < referringObject.ffRefs.length; refItemIx++) {
                    ...
                    if (cachedRefItem) {
                        ...
                        if (m_debug) console.log("m_loadAllReferences: Loading reference: " + m_stringify(ffRefItem));
